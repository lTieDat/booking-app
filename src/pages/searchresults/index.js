...
const SearchResult = () => {
  ...
  const { location: initialLocation, booking: initialBooking } = useQueryParams()
  ...
  const items = ['In-room dining', 'Complimentary Wi-Fi', 'Coffee maker', 'Refrigerator']
    ...
    if (Array.isArray(item)) {
      ...
      setRangeValue((prevRange) =>
        prevRange.some((range) => range[0] === item[0] && range[1] === item[1])
          ? prevRange.filter((range) => range[0] !== item[0] || range[1] !== item[1])
          : [...prevRange, item]
      )
    } else {
          ...
          <div className="search-result__main-display">
            <h3 className="search-result__main-title">
              {location.city !== 'undefined' ? (
                <>
                  {location.city} {location.country}: {hotels.length} {hotels.length > 1 ? 'results' : 'result'} found
                </>
              ) : (
                <>
                  {location.country}: {hotels.length} {hotels.length > 1 ? 'results' : 'result'} found
                </>
              )}
            </h3>
            <div className="search-result__main-itemList">
              {hotels.map((hotel) => (
                <div key={hotel.HotelId} className="search-result__main-itemList-item">
                  {/* rooms left */}
                  <div className="search-result__main-itemList-rooms">
                    {hotel.NumberOfRooms} {hotel.NumberOfRooms > 1 ? 'rooms' : 'room'} left !
                  </div>
                  {/* end rooms left */}
                  <div className="search-result__main-itemList-img">
                    <img src={hotel.images.imgSource} alt={hotel.HotelName} className="search-result__hotel-img" />
                  </div>
                  <div className="search-result__main-itemList-content">
                    <h4 className="search-result__main-itemList-name">{hotel.HotelName}</h4>
                    <div className="search-result__main-itemList-hotelRating">
                      <Rate disabled allowHalf defaultValue={hotel.Rating} />
                      <p>{hotel.Rating}</p>
                    </div>
                    <p className="search-result__main-itemList-hotelDesc">{shortenParagraph(hotel.Description, 100)}</p>
                  ...
                  <div className="search-result__main-itemList-price">
                    <button className="search-result__main-itemList-btn" onClick={handleHotelDetail(hotel.HotelId)}>
                      View Details
                    </button>
                  </div>
                </div>
